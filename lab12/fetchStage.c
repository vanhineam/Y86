 ...
 * @parm con the pipeline control values.
 */
static bool stallF(control con);

/**
 * Determines if the decode stage should be stalled.
 * @param con the pipeline control values.
 */
static bool stallD(control con);

/**
 * Determines if the decode stage should be bubbled.
 * @param con the pipeline control values.
 */
static bool bubbleD(control con);

bool stallF(control con)
{
  bool hazardEicode = con.E_icode == IMRMOVL || con.E_icode == IPOPL;
  bool hazardEdstm = con.E_dstM == con.d_srcA || con.E_dstM == con.d_srcB;
  bool icodeRet = con.D_icode == IRET || con.E_icode == IRET || con.M_icode == IRET;
  return (hazardEicode && hazardEdstm) || icodeRet;
}

bool stallD(control con)
{
  bool e_icode = con.E_icode == IMRMOVL || con.E_icode == IPOPL;
  bool e_dstm = con.E_dstM == con.d_srcA || con.E_dstM == con.d_srcB;
  return e_icode && e_dstm;
}

bool bubbleD(control con)
{
  bool branchMispredict = con.E_icode == IJXX && !con.e_Cnd;
  bool hazardIcode = con.E_icode == IMRMOVL || con.E_icode == IPOPL;
  bool hazardDstm = con.E_dstM == con.d_srcA || con.E_dstM == con.d_srcB;
  bool hazard = hazardIcode && hazardDstm;
  bool icodeRet = con.D_icode == IRET || con.E_icode == IRET || con.M_icode == IRET;
  return branchMispredict || (!hazard && icodeRet);
}
...
void fetchStage(pipelineForward pf, control con)
{
