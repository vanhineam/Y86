...
/**
 * Outputs the starting address in the variable address and the contents of
 * the line array.
 * @param line array of ints to output.
 * @param address the row header
 */
static void dumpLine(int line[WORDSPERLINE], int address);

/**
 * Accesses memory for WORDSPERLINE words and sets the line array to the
 * words from memory.
 * @param line an array with WORDSPERLINE entries, which recieves the data
 * from memory.
 * @param address the starting index to access memory.
 */
static void buildLine(int line[WORDSPERLINE], int address);

/**
 * Compares the contents of prevLine and currLine to determine if they
 * are equal.
 * @param prevLine the previous line with WORDSPERLINE entries.
 * @param currLine the current line with WORDSPERLINE entries.
 * @return true of the two lines are equal.
 */
static int isEqual(int prevLine[WORDSPERLINE], int currLine[WORDSPERLINE]);

/**
 * Copies the contents of cLine into pLine.
 * @param pLine the destination line with WORDSPERLINE entires.
 * @param cLine the current line, with WORDSPERLINE entries.
 */
